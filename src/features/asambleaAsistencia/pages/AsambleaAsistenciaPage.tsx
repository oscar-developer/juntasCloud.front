...
} from '../services/asambleaAsistenciaApi';
import {
...
} from '../services/asambleaAttendanceViewModel';
import type {
...
export function AsambleaAsistenciaPage() {
  ...
  const handleSelectStatus = async (
    ...
    try {
      ...
      setAttendanceRecords((current) => restoreAttendanceRecord(current, row.personaId, previousRecord));
      setRowStateById((current) => ({
        ...current,
        [rowId]: {
          saveState: 'error',
    ...
    <Box sx={{ pb: { xs: 4, md: 0 } }}>
      <Stack spacing={3}>
        <AsambleaAttendanceHeader
          asambleas={orderedAsambleas}
          asambleasLoading={asambleasLoading}
          onSelectAsamblea={handleSelectAsamblea}
          selectedAsamblea={selectedAsamblea}
          selectedAsambleaId={selectedAsambleaId}
          summary={summary}
        ...
        {showNoAsambleas ? (
          <Card elevation={0}>
            <CardContent sx={{ p: { xs: 4, md: 5 }, textAlign: 'center' }}>
              ...
              <Button component={RouterLink} sx={{ mt: 3 }} to={asambleasPagePath} variant="contained">
                Ir a asambleas
              </Button>
            </CardContent>
          </Card>
        ...
        {selectedAsamblea && filteredRows.length > 0 ? (
          isDesktop ? (
            <Card elevation={0}>
